...
namespace lab32 {
	...
	void Proxy::run() {
		...
		/* что есть, то есть: на лекции говорили сделать пайпы и прочие приколы дл€ обработки сигналов, сделаем отдельный поток дл€ обработки SIGINT */
		...
		/* и снова poll, ибо не €сно, как иначе отслеживать происход€щее */
		int selectedDescNum;
		...
		while (!shouldStop() && !cache->ranOutOfMemory()) {
			...
			if (selectedDescNum > 0) {
				...
				if (pollingSet[0].revents == POLLIN) {
					...
					// дл€ передачи разных параметров в поток
			...
			/* больше нет никакого цикла, где можно было бы пон€ть, как там хэндлеры, поэтому есть мапа, позвол€юща€ оценить это*/
			/* удаление мЄртвых обработчиков*/
			cleanDoneHandlers();
...
/*
 *  если добавл€ть нормальную поддержку http 1.1 нкжно сделать:
 *  1) навесить цикл while на  server и client
 *  2) дать им пайпы, чтобы они общались между собой (сообщели о приЄме/ отправке данных)
